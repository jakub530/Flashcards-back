...
sessionSchema.methods.selectNewItem = async function()
{
  ...
  let nextItems = await SessionItem.find({
    session:session._id,
  ...
  if(nextItems.length === 0)
  {
    ...
    nextItems = await SessionItem.find({
      session:session._id,
      bucket:session.state.currentBucket
    })
    console.log("Found Again", nextItems)
  }
  console.log("Looking in bucket:", session.state.currentBucket)
  console.log("Looking for next item:", nextItems)
  const nextItem = _.sample(nextItems)
  console.log("Found next item:", nextItem)
  session.state.currentItem = nextItem
  session.state.itemFlag = "pending";
  if(session.state.currentItem != null)
  {
    session.state.previousItems = session.state.previousItems.filter(item => item.toString() !== session.state.currentItem.toString())
  }
...
sessionSchema.methods.proceedToNextItem = async function()
{
  ...
  sessionItem = await sessionItem.updateBucket(
    session,
    "normal",
  )

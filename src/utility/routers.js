...
const findAllSessionCards = async(sessionId) => {
  ...
  const sets = await Set.find({_id:{$in:
    session.sets
  }})
  const setMap = sets.reduce((acc,curr) =>(acc[curr._id]=curr.name,acc),{});
  console.log(setMap);
  const sessionItems = await SessionItem.find({session:sessionId})
  const sessionCards = await Promise.all(sessionItems.map(async (item) => {
    ...
    return {finished:item.finished,history:item.history,bucket:item.bucket, term:card.term, definition:card.definition, set:setMap[card.set]}
  }));
...
const findSessionCards = async(session) => {
  ...
  const currentCard = await findCardOfSessionItem(session.state.currentItem)
